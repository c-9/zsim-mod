...
//uint64_t OOOCore::access_num;
//uint64_t OOOCore::tlb_access_num;
lock_t OOOCore::paging_switch_lock;
OOOCore::OOOCore(FilterCache* _l1i, FilterCache* _l1d, BaseTlb* _itlb ,
				BaseTlb* _dtlb , g_string& _name, uint32_t _id) :Core(_name),
				l1i(_l1i), l1d(_l1d) , itlb(_itlb) , dtlb(_dtlb) , 
				id(_id), cRec(0, _name)
{
...
inline void OOOCore::load(Address addr) {
	loadAddrs[loads++] = this->TlbTranslate( addr , false);
	access_num++;
}

void OOOCore::store(Address addr) {
	storeAddrs[stores++]=this->TlbTranslate(addr , false , true);
	access_num++;
}
...
inline void OOOCore::bbl(Address bblAddr, BblInfo* bblInfo) {
	...
	bblAddr = this->TlbTranslate(bblAddr , true);
	//std::cout<<"tlb translated end"<<std::endl;
    if (!prevBbl) {
	...
	//std::cout<<"simulate branch prediction"<<std::endl;
    ...
    if (branchPc && !branchPred.predict(branchPc, branchTaken)) {
		...
		//std::cout<<std::hex<<wrongPathAddr<<std::endl;
...
//TLB simulation 
ADDRINT OOOCore::TlbTranslate( ADDRINT virtual_addr , bool is_inst, bool is_write)
{
	futex_lock(&tlb_lock);
	tlb_access_num++;
	BaseTlb* tlb = NULL;
	...
	{
		if( zinfo->addressRandomization)
			virtual_addr = RandomizeAddress(virtual_addr);
			futex_unlock(&tlb_lock);
			return virtual_addr;
	}
	MemReq req;
...
ADDRINT OOOCore::RandomizeAddress(ADDRINT vAddr) {
	...
	debug_printf("randomize translate: vaddr:%d , paddr:%ld",vAddr, pAddr);
        return pAddr;
    }
    return vAddr;
}

//have the function of TLB
