...
static void PopulateLevel(Config& config, const std::string& prefix, std::vector<ProcessTreeNode*>& globProcVector, ProcessTreeNode* parent, uint32_t& procIdx, uint32_t& groupIdx) {
    ...
    while (true) {
        ...
        if (dumpInstrs) {
			...
			//lambda expression: get instruction
...
void CreateProcessTree(Config& config) {
	//rootNode: pid=-1 , gid=-1,inFastFoward=false,inPause=false
	//inSyncFastFoward=false, clockDomain=0,portDomain=0,
	//dumpHeartBeates=0,dumpsResetHeartBeats=false
	//restart=0,mask{},ffipoints{}, syscallBlackListRegex{}
	//patchRoot=NULL
