...
class ReorderBuffer {
    private:
        uint64_t buf[SZ]; //reorder buffer size is SZ
...
class OOOCore : public Core {
    ...
    private:
		...
		BaseTlb* dtlb;
		lock_t tlb_lock;
		static lock_t paging_switch_lock;
        uint32_t id;
    ...
    public:
        OOOCore(FilterCache* _l1i, FilterCache* _l1d, BaseTlb* _itlb 
				, BaseTlb* _dtlb , g_string& _name, uint32_t _id);
		...
		BaseCache* getInsCache()
		{
			return (BaseCache*)(l1i);
		}
		BaseCache* getDataCache()
		{
    ...
    private:
        ...
        //ADDRINT RandomizeAddress(MemReq& req);
        ADDRINT RemapAddress(ADDRINT vaPage);
		ADDRINT TlbTranslate(ADDRINT virtual_addr , bool is_ins , bool is_write = false);
